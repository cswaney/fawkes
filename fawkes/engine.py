...
class MarketMaker(Agent):
    ...
    def default_algo(self, time, book, events):
        ...
        if action is not None:
            ...
            side = action['side']
            if label == 'limit':
                # place limit order at or one tick ahead or behind best price
                pass
            if label == 'cancel':
                # place a cancel order for order on that side
                pass
            if label == 'market':
                pass
            if label == 'replace':
    ...
    def replace_order(self, side, order):
        self.orders[side] = order.copy()

    def cancel_order(self, side):
        self.orders[side] = None

    def add_order(self, side, order):
        assert self.orders['side'] is None, "Agent already has a {} order".format(side)
        self.orders[side] = order.copy()
